...
jQuery(($) => {
   ...
   * @type {Boolean}
   */
  const askForPrintPosition = Boolean(parseInt(wcpn.ask_for_print_position));
  ...
  function doBulkAction(event) {
    ...
    if (!orderIds.length) {
      showAdminNotice(wcpn.strings.no_orders_selected);
      return;
    }
  ...
  function showDialog(orderIds, dialog) {
    ...
    const url = `${wcpn.ajax_url}?${$.param(data)}`;

    /* disable background scrolling */
    $('body').css({overflow: 'hidden'});

    tb_show(wcpn.strings.dialog[dialog], url);
  }
  ...
  function printLabel(data) {
    ...
    request.afterDone = (response) => {
      const isDisplay = wcpn.download_display === 'display';
      const isDownload = wcpn.download_display === 'download';
